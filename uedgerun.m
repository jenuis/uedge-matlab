    ...
    methods
        ...
        function file_run = script_run_gen(self, varargin)
            ...
            contents = {
                ...
                'ur = UeRun()', ...
                'status = ur.converge(ii1max=ii1max, t_stop=t_stop, ftol_min=ftol_min, dt_kill=dt_kill, savedir=".", savefname=profile_tmp)', ...
                'profile_tmp += "_last_ii2.hdf5"',...
                'profile_fail = profile_save.replace("_successful", "")',...
                'if status == False:',...
                '    os.rename(profile_tmp, profile_fail.replace(".hdf5", "_max-iter.hdf5"))', ...
                ['    print("' disp_prefix 'Faild to converge: max iteration reached!"); sys.exit(101)'],...
                '',...
                ['assert hasattr(ur, "fnrm_old"),  "' disp_prefix 'Faild to converge: init step!"'],...
                '',...
                'dtreal_fail = ur.classvars["dtrealfail"]', ...
                'flag = len(dtreal_fail) > 1 and dtreal_fail[-1] < dt_kill',...
                'if flag:',...
                '    os.rename(profile_tmp, profile_fail.replace(".hdf5", "_min-dt.hdf5"))', ...
                ['    print("' disp_prefix 'Failed to converge: min dt reached!"); sys.exit(102)'],...
                '',...
                'flag = bbb.dt_tot >= t_stop', ...
                'if flag:', ...
                '    os.rename(profile_tmp, profile_fail.replace(".hdf5", "_max-time.hdf5"))', ...
                ['    print("' disp_prefix 'Failed to converge: max time reached!"); sys.exit(103)'],...
                '',...
                ['assert bbb.iterm == 1, "' disp_prefix 'bbb.iterm != 1"'], ...
                'flag = ur.fnrm_old < ftol_min', ...
                'if not flag:', ...
                '    os.rename(profile_tmp, profile_fail.replace(".hdf5", "_ftol-fail.hdf5"))', ...
                ['    print("' disp_prefix 'Failed to converge: ftol_min not reached!"); sys.exit(104)'],...
                '',...
